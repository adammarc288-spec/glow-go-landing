            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => {
                    const colorAttr = item.attributes?.find((a) => a.key === "Farbe")?.value;
                    const colorOpt = item.selectedOptions.find((o) => o.name === "Farbe")?.value;
                    const color = colorAttr ?? colorOpt ?? "";
                    const imgUrl =
                      item.product.node.images?.edges?.[0]?.node?.url ?? "";
                    return (
                      <div key={item.variantId} className="flex gap-4 p-3 rounded-2xl bg-muted/50">
                        <div className="w-20 h-20 bg-secondary/20 rounded-xl overflow-hidden flex-shrink-0 relative">
                          {imgUrl && (
                            <img
                              src={imgUrl}
                              alt={`${item.product.node.title} – ${color}`}
                        ...
                        <div className="flex flex-col items-end gap-2 flex-shrink-0">
                          <Button
                            ...
                            aria-label={`${color || "Artikel"} entfernen`}
                          >
